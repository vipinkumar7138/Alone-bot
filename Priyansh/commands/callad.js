...
module.exports.handleReply = async function({ api, args, event, handleReply, Users }) {
  try {
    ...
    switch (handleReply.type) {
      ...
      case "calladmin": {
        if (s.length == 0) {
          api.sendMessage({ body: `[📌] Feedback from admin ${name} to you:\n\n[💬] Content: ${(event.body) || "no reply 🌸 "}\n\n» Reply to this message if you want to continue sending  reports to my owner 𝑱𝑼𝑳𝑴𝑰 𝑱𝑨𝑨𝑻`, mentions: [{ tag: name, id: event.senderID }] }, handleReply.id, (e, data) => global.client.handleReply.push({
            name: this.config.name,
            author: event.senderID,
            messageID: data.messageID,
            type: "reply"
          ...
          api.sendMessage({ body: `[📌] Feedback from admin ${name} to you:\n\n[💬] Content: ${(event.body) || "only files with no reply 🌸 "}\n[💌] Admin files sent to you\n\n» Reply to this message if you want to continue sending reports to my owner 𝑱𝑼𝑳𝑴𝑰 𝑱𝑨𝑨𝑻`, attachment: l, mentions: [{ tag: name, id: event.senderID }] }, handleReply.id, (e, data) => global.client.handleReply.push({
            name: this.config.name,
            author: event.senderID,
            messageID: data.messageID,
            type: "reply"
...
module.exports.run = async function({ api, event, Threads, args, Users }) {
  try {
    ...
    api.sendMessage(`[🤖] - Bot has just successfully sent your message to ${soad} to my owner 𝑱𝑼𝑳𝑴𝑰 𝑱𝑨𝑨𝑻 🍄 \n[⏰] - Time: ${gio}`,
      event.threadID,
      () => {
        var idad = global.config.ADMINBOT;
        if (s.length == 0) {
